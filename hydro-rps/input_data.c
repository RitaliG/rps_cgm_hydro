...
#define ID_NVAR_MAX 16  /*< Max number of field variables accessible simultaneously */
#ifndef ID_NZ_MAX
 #define ID_NZ_MAX   4  /**< Maximum size (in the 3rd dimension) of the
                             input buffer. */
#endif

/* Each structure store everything from input file for each variable like rho, prs, etc */
...
#ifdef NO_DISK
static double ***Vorig[ID_NVAR_MAX];  /**< Entire Data from dbl */
#endif
...
/* --------------------------------------------------------------------- */
  ...
  success = 0;
  while(!success){
    if( fgets(sline,512,fp) == NULL ) {print("Unexpected exit! %s:%d\n", __FILE__, __LINE__); QUIT_PLUTO(1);}
...
/* --------------------------------------------------------------------- */
  ...
  success = 0;
  while(!success){
    dummy = fgetpos(fp, &file_pos);
    if( fgets(sline,512,fp) == NULL ) {print("Unexpected exit! input_data.c:%d\n",164); QUIT_PLUTO(1);}
...
/* ---------------------------------------------------------- */
  ...
  #ifdef NO_DISK
  Vorig[indx] = ARRAY_3D(id->nx3, id->nx2, id->nx1, double);  
  #endif 
...
/* --------------------------------------------------------------------- */
  ...
  if (id->geom == GEOMETRY) {  
    ...
    /* same coordinate system: rotation about the x-axis */
    ...
    y_prime =  cos(theta) * ( x2-z_offset ) + sin(theta) * ( x3-z_offset );
    z_prime = -sin(theta) * ( x2-z_offset ) + cos(theta) * ( x3-z_offset );
    x2 = y_prime + z_offset; x3 = z_prime + z_offset;
    /* end of changes for angle */
  ...
  if (id->geom == GEOMETRY) {  
    ...
    */
    ...
    // rotated coordinates in galaxy's frame of reference ------------
    x_prime =  x1 * cos(x2);
    y_prime =  cos(theta) * ( x1*sin(x2) ) + sin(theta) * ( x3-z_offset );
    z_prime = -sin(theta) * ( x1*sin(x2) ) + cos(theta) * ( x3-z_offset );
 
    x1 = x_prime; x2 = y_prime; x3 = z_prime + z_offset;
    /* end of changes for angle */
  }else{
    printLog ("! InputDataInterpolate(): invalid or unsupported coordinate transformation.\n");
    QUIT_PLUTO(1);
...
/* --------------------------------------------------------------------- */
  ...
  #ifdef NO_DISK
  if (isnan(Vorig[indx][0][0][0]) ){
   //printLog("Field ID %d: Starting reading and allocating entire array!\n", indx);
    FILE *fp;
    double udbl;
    float  uflt;
    fp = fopen(id->fname,"rb");
    fseek(fp, id->offset, SEEK_SET);
      ...
      for (k = 0; k < id->nx3; k++){ 
        for (j = 0; j < id->nx2; j++){
          for (i = 0; i < id->nx1; i++){
            ...
            Vorig[indx][k][j][i] = (double)uflt;
      }}}
    }
    fclose(fp);
    //printLog("Field ID %d: Finished reading and allocating entire array!\n", indx);
  }
  #endif
...
*/
  ...
  for (k = 0; k < kmax   ; k++){   /* Read at most kmax planes */
    for (j = 0; j < id->nx2; j++){
      for (i = 0; i < id->nx1; i++){
        id->Vin[k][j][i] = Vorig[indx][kslice+k][j][i];
  }}}
